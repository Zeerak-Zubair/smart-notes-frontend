...
const EditNotebook = () => {
  ...
  const { id } = useParams(); //notebookId
  const navigate = useNavigate();

  // Predefined color palette (same as AddNotebook)
  const colorPalette = [
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      const notebook = await NotebooksApiService.updateNotebook(
        id!,
        title,
        description,
        color
      );
      console.log("Update Notebook API response:", notebook);
      setSuccess(true);
      ...
      setError(err.message || "An error occurred during updating notebook");
    } finally {
      setIsLoading(false);
    }
  };
        ...
        <Flex direction="column" gap="4">
          ...
          {success && (
            <Callout.Root color="green">
              <Callout.Text>
                Notebook updated successfully! Redirecting...
              </Callout.Text>
            </Callout.Root>
          )}
